...
const {assignTokens,getallTx,getSaleInfo, whitelisted,getNumberMinted, getLeft,updateMint} = require("./db/db");
...
app.get("/api/v1/sale/:address",async (req,res)=> { 
    ...
    if (REMAINING_NFT >= process.env.COLLECTION_SIZE)
     {
        ...
        res.status(401).send({sale:"SOLD OUT"})
     ...
     { 
        ...
        { 
            res.status(401).send({"sale":"Sale Limit Exceeded"})
        }

        else 
        { 
            res.send({"sale":ELIGABLE_AMOUNT})
        }
            }
        else 
        { 
        res.send({"sale":"Sale does not have a limit"})
        }
    ...
    res.send({sale:"Public sale is live!"})
     }
     else if (SALE_INFO[0].public == false && SALE_INFO[0].presale == false)
     { 
    
        res.status(401).send({sale:"Sale has ended"})
    }
   ...
   { 
    ...
    { 
        ...
        {   
            ...
            try 
            { 
                const assignedTokens = await assignTokens(1,req.body.address.toLowerCase(),req.body.hash)

                const mintResult = await mintNFT(req.body.address.toLowerCase(),assignedTokens[0].id)
                console.log(`${mintResult.token} has been minted to the address of ${req.body.address}`)
            ...
            console.log(req.body.address.toLowerCase(),"<------ There was an issue with token ID", assignedTokens[0].id)
        ...
        if (errs)
        { 
            res.send({status:"Tokens being processed!"})
        }

        else 
        { 
            updateMint(req.body.hash,req.body.address,"success")
            res.send({"success":"Tokens have been proccessed!"})
        }
    }
