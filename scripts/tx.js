   ...
   {       
    ...
    return new Promise( async (resolve,reject)=> { 
        ...
        { 
            ...
            reject({err:`There was an error proccessing payment for hash: ${hash} has been added to the tx queue`})
        }
    })
...
const checkMatch = async (element) => { 
    ...
    return new Promise(async (resolve,reject) => { 
        ...
        if (ETHER_PAID == NFT_PRICE * element.quantity && txData.to.toLowerCase() == process.env.TO.toLowerCase())
        { 
...
const immutable = (element) => { 
    ...
    return new Promise((resolve,reject)=> { 
        ...
        fetch(`https://api.x.immutable.com/v1/transfers/${element.hash.toString()}`, options)
        ...
        if (ETHER_PAID == NFT_PRICE * element.quantity && TO.toLowerCase() == process.env.TO.toLowerCase())
        {
